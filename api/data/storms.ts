...
async function fetchHangzhouWeather(lat = HANGZHOU.lat, lng = HANGZHOU.lng, name = HANGZHOU.name) {
  const url =
...
async function fetchHangzhouWeatherPackage() {
  const url =
    `https://api.open-meteo.com/v1/forecast?latitude=${HANGZHOU.lat}&longitude=${HANGZHOU.lng}` +
    '&current=wind_speed_10m,wind_direction_10m,wind_gusts_10m,pressure_msl' +
    '&hourly=wind_speed_10m,wind_direction_10m,wind_gusts_10m,pressure_msl,precipitation&forecast_hours=8&timezone=Asia%2FShanghai'
  const data = await requestJson<OpenMeteoResponse>(url)
  ...
  const timeline: WeatherTimelinePoint[] = data.hourly.time.map((time, index) => ({
    time: `${time}:00+08:00`,
...
async function fetchWindVectors() {
  ...
  const result = await Promise.all(
    coords.map(async ([lat, lng]) => {
      const weather = await fetchHangzhouWeather(lat, lng, `${lat.toFixed(1)},${lng.toFixed(1)}`)
      return {
        lat,
        lng,
...
async function fetchLiveStormsFromPanahon(hangzhouWeather: WeatherSnapshot, windVectors: WindVectorPoint[]) {
  ...
  return data.map((cyclone, index) => {
    ...
    const maxWind = Math.max(...points.map((item) => item.windSpeedKts))
    const minPressure = Math.min(...points.map((item) => item.pressureHpa))
    const latestMeta = intensityMeta[orderedEntries[Math.max(0, orderedEntries.length - 1)].cyclone_type] ?? intensityMeta.TD
    const radius = Number(orderedEntries[Math.max(0, orderedEntries.length - 1)].radius || '0')
      ...
      windCircles:
        radius > 0
          ? [
              {
                level: '7',
                radiusKm: { ne: radius, se: radius, sw: radius, nw: radius },
              },
            ]
          : [],
      affectedAreas: [
...
async function loadLiveBundle(): Promise<LiveBundle> {
  if (process.env.VITEST) {
    return createFallbackLiveBundle()
  }
  ...
  const { snapshot: hangzhouWeather, timeline: hangzhouTimeline } = await fetchHangzhouWeatherPackage()
...
export async function getLiveStorms(): Promise<LiveStormsResponse> {
  const bundle = await loadLiveBundle().catch(() => createFallbackLiveBundle())
...
export async function getStormById(stormId: string) {
  const bundle = await loadLiveBundle().catch(() => null)
...
export async function getTimeline(stormId: string) {
  return (await getStormById(stormId))?.points ?? []
}

export function getHistory(filters: HistoryQuery): HistoryResponse {
...
export function compareStorms(ids: string[]): CompareResponse {
  return {
    ids,
